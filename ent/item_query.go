...
type ItemQuery struct {
	...
	// eager-loading edges.
	withItemDescription *ItemDescriptionQuery
	withItemVariation   *ItemVariationQuery
	withItemGroup       *ItemGroupQuery
...
// QueryItemDescription chains the current query on the "item_description" edge.
func (iq *ItemQuery) QueryItemDescription() *ItemDescriptionQuery {
	query := &ItemDescriptionQuery{config: iq.config}
...
// QueryItemVariation chains the current query on the "item_variation" edge.
func (iq *ItemQuery) QueryItemVariation() *ItemVariationQuery {
	query := &ItemVariationQuery{config: iq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryItemGroup chains the current query on the "item_group" edge.
func (iq *ItemQuery) QueryItemGroup() *ItemGroupQuery {
	query := &ItemGroupQuery{config: iq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *ItemQuery) Clone() *ItemQuery {
	...
	return &ItemQuery{
		...
		withItemDescription: iq.withItemDescription.Clone(),
		withItemVariation:   iq.withItemVariation.Clone(),
		withItemGroup:       iq.withItemGroup.Clone(),
...
// WithItemDescription tells the query-builder to eager-load the nodes that are connected to
// the "item_description" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ItemQuery) WithItemDescription(opts ...func(*ItemDescriptionQuery)) *ItemQuery {
	query := &ItemDescriptionQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withItemDescription = query
	return iq
}

// WithItemVariation tells the query-builder to eager-load the nodes that are connected to
// the "item_variation" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ItemQuery) WithItemVariation(opts ...func(*ItemVariationQuery)) *ItemQuery {
	query := &ItemVariationQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withItemVariation = query
	return iq
}

// WithItemGroup tells the query-builder to eager-load the nodes that are connected to
// the "item_group" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ItemQuery) WithItemGroup(opts ...func(*ItemGroupQuery)) *ItemQuery {
	query := &ItemGroupQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withItemGroup = query
	return iq
}
...
func (iq *ItemQuery) sqlAll(ctx context.Context) ([]*Item, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [3]bool{
			iq.withItemDescription != nil,
			iq.withItemVariation != nil,
			iq.withItemGroup != nil,
		}
	)
	...
	if query := iq.withItemDescription; query != nil {
		...
		query.withFKs = true
		query.Where(predicate.ItemDescription(func(s *sql.Selector) {
			s.Where(sql.InValues(item.ItemDescriptionColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.item_item_description
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "item_item_description" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "item_item_description" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ItemDescription = n
		}
	}

	if query := iq.withItemVariation; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Item)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ItemVariation = []*ItemVariation{}
		}
		query.withFKs = true
		query.Where(predicate.ItemVariation(func(s *sql.Selector) {
			s.Where(sql.InValues(item.ItemVariationColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.item_item_variation
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "item_item_variation" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "item_item_variation" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ItemVariation = append(node.Edges.ItemVariation, n)
		}
	}

	if query := iq.withItemGroup; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Item, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.ItemGroup = []*ItemGroup{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(item.ItemGroupPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, iq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "item_group": %v`, err)
		}
		query.Where(itemgroup.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "item_group" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ItemGroup = append(nodes[i].Edges.ItemGroup, n)
			}
		}
	}
